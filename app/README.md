...
### Manter o site no ar (systemd)

Rodar `php artisan serve` com `nohup` cai quando a máquina reinicia ou o processo morre, e o proxy passa a devolver **502 Bad Gateway**. Para o servidor Linux, use os serviços da pasta [`deploy/`](../deploy/):

- `seer-web`: o servidor web (`artisan serve`, porta 8000). Sobe no boot e reinicia sozinho se cair.
- `seer-queue`: o worker da fila (importações e relatórios).
- `seer-saude` (timer, a cada minuto): chama `/up` e reinicia o `seer-web` se o site não responder.

Instalar (uma vez, no servidor):

```bash
sudo sh deploy/instalar-servicos.sh
```

O script para os processos soltos com `nohup`, instala os serviços e confere a resposta. Os arquivos assumem o usuário `matheus` e o caminho `/home/matheus/InovaApps2026/app`; ajuste se for diferente. Depois disso:

```bash
systemctl status seer-web seer-queue          # estado
journalctl -u seer-web -n 50 --no-pager       # log do servidor
sudo systemctl restart seer-web seer-queue    # reiniciar (ex.: depois de mudar o .env)
```

O deploy pelo GitHub continua igual: o `queue:restart` faz o worker sair e o systemd o sobe de novo com o código novo.
