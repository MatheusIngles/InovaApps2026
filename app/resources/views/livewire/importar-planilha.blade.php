<div class="pl">
    <section class="ui-card ui-pad">
        ...
        <p class="ui-muted">Cada linha é um cliente em um mês. Depois do envio, escolha a coluna de cada campo. Meses já importados só têm os valores atualizados.</p>
    ...
    @if ($cabecalhos)
        <section class="ui-card ui-pad">
            ...
            <p class="ui-muted">Só o código do cliente é obrigatório. Sem dado de uso da plataforma, o cliente não é penalizado.</p>
    ...
    @if ($resultado)
        <section class="ui-card ui-pad" role="status">
            ...
            <p class="ui-about">{{ $resultado['clientes'] }} clientes · {{ $resultado['meses'] }} meses de métricas · {{ $resultado['nps'] }} pesquisas de NPS{{ $resultado['ignoradas'] ? ' · '.$resultado['ignoradas'].' linhas ignoradas (código ou mês inválido)' : '' }}.</p>
        </section>
    @endif
</div>
