@extends('layouts.guest', ['title' => 'Criar Nova Conta'])

@section('content')
    <div class="mb-6 text-center">
        <h1 class="text-2xl font-bold tracking-tight text-white">Criar nova conta</h1>
        <p class="text-sm text-slate-400 mt-1">Preencha os campos abaixo para se cadastrar</p>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        ...
        {{-- Campo Senha --}}
        <div>
            <label for="password" class="block text-xs font-medium text-slate-300 mb-1.5">Senha (mínimo 6 caracteres)</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-500">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div>
            <label for="password_confirmation" class="block text-xs font-medium text-slate-300 mb-1.5">Confirmação da Senha</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-500">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex items-start pt-1">
            <input type="checkbox" 
                   ...
                   name="terms" 
                   required
                   class="w-4 h-4 mt-0.5 rounded border-slate-800 bg-slate-950 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-slate-900 cursor-pointer">
            <label for="terms" class="ml-2 text-xs text-slate-400 select-none cursor-pointer">
                Concordo com os <a href="#" class="text-indigo-400 hover:text-indigo-300 underline">Termos de Uso</a> e as diretrizes do InovaApps 2026.
            </label>
        </div>
        @error('terms')
    ...
    <div class="mt-6 pt-5 border-t border-slate-800/80 text-center">
        <p class="text-xs text-slate-400">
            Já possui uma conta? 
            <a href="{{ route('login') }}" class="font-semibold text-indigo-400 hover:text-indigo-300 transition-colors ml-1">
                Fazer login
            </a>
        </p>
    </div>
