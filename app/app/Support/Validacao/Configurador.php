...
class Configurador
{
    ...
    public static function contexto(Company $company): string
    {
        ...
        $linhas[] = '';
        $linhas[] = 'VARIÁVEIS (separação 0,5 = nada, 1,0 = perfeita; antecedência mediana em meses; alarme falso = % de retidos com a variável ≥ 50%):';
        ...
        $linhas[] = '';
        $linhas[] = 'VARIÁVEIS FORA DO SCORE: são colunas guardadas da planilha que o risco atual não usa (ele só soma os sinais acima). A separação é o AUC, calculado com a média dos 3 últimos meses de cada cliente, comparando cancelados (mês antes da saída) e retidos (mês mais recente); mostra correlação, não causa.';
        ...
        $linhas[] = "Cortes recomendados: médio {$rec['limiares']['medio']} (alarme falso até 30%), alto {$rec['limiares']['alto']} (até 8%), crítico {$rec['limiares']['critico']} (até 2%).";
    ...
    public static function textoLocal(Company $company): string
    {
        ...
        return "**Prioridade das métricas.** As que mais separam quem cancelou de quem ficou: {$top}. {$desligadas}\n\n"
            ...
            .'**Cuidados.** São poucos cancelamentos e a análise usa os mesmos dados da calibração; o risco ordena o atendimento, não é probabilidade de cancelamento. Tudo pode ser ajustado em Configurações.';
    }
}
