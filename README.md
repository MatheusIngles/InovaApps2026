...
Em **Configurações › Prioridades** ficam, na mesma lista, os 8 sinais padrão (ordem por arraste, pesos em "Editar pesos") e, logo abaixo, as métricas da empresa, da maior para a menor prioridade: é só mudar o peso ou desligar a métrica e salvar, que a ordem se ajusta e a atenção é recalculada. Em **Configurações › Métricas**, elas aparecem em cartões recolhíveis (nome, tipo, estado, participação e resumo da faixa) e o painel **Participação de cada fator na atenção** mostra, em %, quanto cada um pesa. Os sinais padrão e as métricas da empresa entram na **mesma conta**: a participação é o peso de cada fator dividido pela soma de todos os pesos, então ligar uma métrica nova reduz a participação relativa dos demais.
